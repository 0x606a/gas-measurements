...
interface Aggregator{
    function addOracle(address oracleAddress, address[] calldata devicesAddresses) external ;
    function sendDataRequest(bytes32 UID, address userAddr, address iotDeviceAddr, uint numOfOracles) external ;
    function oracleResponse(bytes calldata response) external ;
    function reportReputationScore(address reputationSC, address[] calldata oracleAddress, bytes[] calldata dataHashes) external ;
}
